...
class CrossModalityIdentitySampler(Sampler):
    def __init__(self, dataset, input_list, p_size, k_size):
        self.dataset = dataset
        self.p_size = p_size
        ...
        id_list = input_list
        reorder=[]
        while len(set(id_list))>=p_size:
            add= random.sample(set(id_list), p_size)
            for i in add:
                id_list.remove(i)
            reorder.extend(add)
        self.reorder = reorder
        self.length = len(reorder)
    ...
    def __iter__(self):
        ...
        for start in range(0, self.length , self.p_size):
            selected_ids = self.reorder[start:start + self.p_size]
            ...
            for identity in selected_ids:
                ...
                sample.extend(s)
                self.id2idx_rgb[identity].remove(s)
            ...
            for identity in selected_ids:
                ...
                sample.extend(s)
                self.id2idx_ir[identity].remove(s)
